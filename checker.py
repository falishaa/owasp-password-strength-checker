...
def load_blocklist(filepath: str = "common_passwords.txt") -> set:
    """Load known common/breached passwords into a set for fast lookup."""
    try:
...
def check_composition(password: str) -> dict:
    rules = {
        ...
        "symbol": (r"[^\w\s]", "Password must contain at least one special character (e.g. !, @, #, $)."),
    }
    errors = []
    points = 0
...
def check_blocklist(password: str, blocklist: set) -> dict:
    ...
    return {
        "pass": not is_common,
        "error": "Password is too common and appears in known breached password lists." if is_common else None,
    }
...
def evaluate_password(password: str, blocklist: set) -> dict:
    ...
    return {"verdict": verdict, "score": total_points, "errors": errors}